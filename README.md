...
The daemon also tails two more perception sources: **Codex CLI rollouts** (only new events after first run) and **macOS `log stream`** for crashes / system incidents (stored as coalesced summaries, never raw streams). Both default on; kill switches `NAVI_CODEX_SOURCE=false` and `NAVI_OS_LOG_SOURCE=false`.
